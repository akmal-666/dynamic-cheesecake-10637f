  ...
  const callMikrotik = useCallback(async (path, method = 'GET', data = null) => {
    ...
    try {
      ...
      if (host === 'localhost' || host === '127.0.0.1') {
        endpoint = '/.netlify/functions/mikrotik'; // local: netlify dev
      } else if (host.includes('netlify.app')) {
        endpoint = '/.netlify/functions/mikrotik'; // Netlify
      } else if (host.includes('vercel.app') || host.includes('vercel.com')) {
        endpoint = '/api/mikrotik'; // Vercel
      } else {
        endpoint = '/mikrotik'; // Cloudflare Pages
      }
